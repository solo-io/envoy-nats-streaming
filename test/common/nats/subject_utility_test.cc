...
TEST_F(NatsSubjectUtilityTest, RandomChild) {
  ...
  auto actual_subject = subject_utility_.randomChild("_INBOX", token_generator);
  EXPECT_EQ(expected_subject, actual_subject);
}
...
TEST_F(NatsSubjectUtilityTest, RandomGrandchild) {
  ...
  auto actual_subject = subject_utility_.randomChild(
...
TEST_F(NatsSubjectUtilityTest, ChildWildcard) {
  ...
  auto actual_subject =
      subject_utility_.childWildcard("_INBOX.M2kl72gBUTGH12kgXu5c9i");
  EXPECT_EQ(expected_subject, actual_subject);
}
